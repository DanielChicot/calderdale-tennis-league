...
describe('home load', () => {
  ...
  it('groups the current season divisions Mens/Ladies/Mixed and counts stats', async () => {
    ...
    vi.mocked(listClubs).mockResolvedValue([
      { id: 1, slug: 'cragg-vale', name: 'Cragg Vale' },
      { id: 2, slug: 'todmorden', name: 'Todmorden' },
    ]);
    const result = await load({} as never);
    ...
    expect(result.stats).toEqual({ divisions: 2, clubs: 2, seasons: 1 });
  });
});
